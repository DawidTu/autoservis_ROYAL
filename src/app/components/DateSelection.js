      ...
      <div className="relative flex-1">
        ...
        <Menu.Button className='dropdown-btn w-full h-full flex flex-col justify-center items-center xl:items-start xl:pl-8'>
          <div className="flex flex-col xl:flex-row items-center xl:gap-x-2 gap-y-2 xl:gap-y-0">
            ...
            <div className="text-[15px] uppercase font-bold">Vyber dátum</div> 
          </div>
          <div className="flex items-center gap-x-3 xl:ml-6">
          <div className="text-[13px] font-medium text-secondary">
            {date?.[0]?.startDate && format(date[0].startDate, 'dd/MM/yyyy')}</div>
            <FaArrowRightLong className="text-accent text-[12px]"/>
            <div className="text-[13px] font-medium text-secondary">
              {date?.[0]?.endDate ? (
                <div>{format(date[0].endDate, 'dd/MM/yyyy')}</div>
              ) : (
                <div>{date?.[0]?.startDate && format(date[0].startDate, 'dd/MM/yyyy')}</div>
              )}
            </div>
          </div>
        </Menu.Button>
